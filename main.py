...
# -------------------------------------------------------
# CREATE APP — MUST COME FIRST BEFORE ANY ROUTES
# -------------------------------------------------------
app = FastAPI()
...
def upload_to_drive(file_path: str, title: str, folder_id: str, job_id: str | None = None):
    ...
    metadata = {"name": f"{title}.mp4", "parents": [folder_id]}
...
# -------------------------------------------------------
# *** NEW FIXED ENDPOINT ***
# NORMALIZES ZAPIER’S SEGMENTS INTO A REAL ARRAY
# -------------------------------------------------------
...
async def analyze_and_clip(request: Request, background_tasks: BackgroundTasks):
    ...
    # ----- FIX #1: Normalize Zapier’s weird expansion -----
    ...
    # Case 2: Zapier wrapped it as an object with numeric keys
        ...
        for key, value in raw_segments.items():
            try:
                ...
                continue
        normalized_segments = extracted

    # Case 3: It's a stringified JSON
...
You must choose up to {max_clip_count} clips.

Rules:
- Use ONLY the segments I provide.
- Video length is {video_total_len_sec} seconds.
- Each clip must be between {min_clip_len_sec} and {max_clip_len_sec} seconds.
- Prefer hooks and educational moments.
            ...
            r = requests.post(
                ...
                json={
                    ...
                    "messages": [
                        ...
                        {"role": "user", "content": json.dumps(normalized_segments)}
                    ]
                },
                timeout=45
            )
