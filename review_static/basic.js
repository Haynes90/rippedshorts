...
setInterval(async()=>{if(!current||busy||document.hidden)return;const id=current.project_id;try{const updated=await api('/projects/'+id);if(busy||current?.project_id!==id)return;const changed=JSON.stringify(updated.lanes)!==JSON.stringify(current.lanes);current=updated;if(changed)render();else{$('project-status').textContent=`${current.stage.replaceAll('_',' ')} · ${current.eta_label}`;$('retry').hidden=!(current.status==='error'||(current.status==='awaiting_review'&&!current.lanes.shorts.length))||current.expired;}}catch(e){notice(e.message);}},12000);
action(dashboard);
