...
function render(){
  ...
  for(const id of ['approve','reject'])$(id).disabled=current.expired||(id==='approve'&&current.selection_version<2)||['queued','rendering','rendered'].includes(c.status);
...
$('rebuild').onclick=()=>action(async()=>{await api(`/projects/${current.project_id}/rebuild`,{method:'POST'});notice('Rebuilding from saved sections; prior decisions are archived.');await openProject(current.project_id);});
...
setInterval(async()=>{if(!current||busy||document.hidden)return;const id=current.project_id;try{const updated=await api('/projects/'+id);if(busy||current?.project_id!==id)return;const changed=JSON.stringify(updated.lanes)!==JSON.stringify(current.lanes)||updated.status!==current.status||updated.can_rebuild!==current.can_rebuild;current=updated;if(changed)render();else{$('project-status').textContent=`${current.stage.replaceAll('_',' ')} · ${current.eta_label}`;$('retry').hidden=!(current.status==='error'||(current.status==='awaiting_review'&&!current.lanes.shorts.length))||current.expired;}}catch(e){notice(e.message);}},12000);
action(dashboard);
